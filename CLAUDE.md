...
This project has two parallel entry points that share the same core logic:
...
3. **`falconpy_auth.py`** — shared credential resolution: CLI args → `~/.falconpy/credentials` (INI, supports named profiles) → env vars (`FALCON_CLIENT_ID`, `FALCON_CLIENT_SECRET`).

4. **`index.html`** — single-file frontend (~1500 lines of vanilla JS + CSS). Connects to the SSE stream, renders namespaces as collapsible sections with per-image CVE tables, supports dark/light theme toggle and client-side filtering.
...
### FQL severity filter caveat

The `--severity` / `severity` query param filters via FQL (`+severity:'HIGH'`) but this only returns CVEs at *exactly* that level. The `SEVERITY_ORDER` dict (`CRITICAL:0 … LOW:3`) is used post-fetch to implement "at or above" filtering when rendering the report.
